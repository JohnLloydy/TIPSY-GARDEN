    ...
    const HandlerSubmit = async (value) => {
        ...
        try {
            const newObject = {
                ...
                cartItems,
                subTotal,
                shippingfee: Number((subTotal.toFixed(2))),
                totalAmount: Number(Number(subTotal) + Number((subTotal / 100) * 10).toFixed(2)),
                email: JSON.parse(localStorage.getItem("auth"))._id
            }
    ...
    return (
        <LayoutApp>
            ...
            <div className="subTotal">
                <h2>Sub Total: <span>P {(subTotal).toFixed(2)}</span></h2>
                <Button onClick={() => setBillPopUp(true)} className='add-new'>Create Invoice</Button>
            </div>
            <Modal title="Delivery Details" visible={billPopUp} onCancel={() =>setBillPopUp(false)} footer={false}>
            <Form layout='vertical' onFinish={HandlerSubmit}>
                        ...
                        <div className="total">
                            <span>SubTotal: P {(subTotal.toFixed(2))}</span>
                            <span>Shipping Fee: P{((subTotal / 100) * 10).toFixed(2)} </span>
                            <h3>Total: P{Number(subTotal) + Number((subTotal / 100) * 10).toFixed(2)}</h3>
                        </div>
                        <div className='form-btn-add'>
                            <Button htmlType='submit' className='add-new'>BUY NOW</Button>
                        </div>
                    </Form>
            </Modal>
        </LayoutApp>
